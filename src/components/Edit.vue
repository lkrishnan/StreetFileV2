    	...
    	methods: {
			...
			edit( type ){
				...
				if( Object.keys( change_row ).length > 0 ){
					...
					const add_admkey = Object.keys( change_row ).some( key => { 
					...
					if( type === "push_alias" ){
						const { objectid, created_date, last_edited_date, ...temp } = _this.stinfo.legal[ 0 ]

						this.$store.dispatch( "update", {
							insert: { 
								data: JSON.stringify( ObjToUppercase( { ...temp, ...change_row, ...{ aliaslegalflag: "L" } } ) )
							},
							update: {
								filter: _this.new_stinfo[ 0 ].objectid, 
								data: JSON.stringify( ObjToUppercase( { aliaslegalflag: "A" } ) ) 
							}
						...
						this.$store.dispatch( "update", {
							update: {
								filter: _this.new_stinfo[ 0 ].objectid, 
								data: JSON.stringify( ObjToUppercase( change_row ) ) 
							}
